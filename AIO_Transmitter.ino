...
// ---- Cached + debounced MCP23017 input state --------------------------------
// Every mcp.digitalRead() is a full I2C transaction (address, register, read,
// stop). loop() was issuing 16-22 of them per iteration, which at the current
// loop rate is thousands per second and milliseconds of pure I2C stall each
// pass. readGPIOAB() fetches all 16 inputs in ONE transaction instead, and
// everything else reads from this cache via mcpBtn(). Implementations sit
// next to readButtons() further down; only the shared state lives here.
//
// Debouncing matters now too: at the old ~30Hz loop the 33ms period masked
// contact bounce for free. At a few hundred Hz it no longer does, so a single
// physical press would otherwise register as several edges. A bit is only
// committed once it has held the same value for MCP_DEBOUNCE_MS.
uint16_t mcpStable = 0xFFFF;              // 1 = released (pull-up), 0 = pressed
unsigned long mcpBitChangeTime[16] = {0};
const unsigned long MCP_DEBOUNCE_MS = 15;
...
#define PIN_BUZZER      2
// SYS_CENTRE was GPIO3 on the original ESP32 WROOM test rig, and separately
// gained a "Power Button" (GPIO21) during the C6 port instead of being moved
// to the real button's actual pin — leaving two names for the same physical
// button. Consolidated onto the real, wired GPIO: short press
// confirms/selects (or wakes from sleep); held ~1s during an active session
// terminates it; held 3s from Home/Apps Menu opens the shutdown confirm; also
// the deep/light-sleep wake source.
#define SYS_CENTRE      21
...
// Power button (SYS_CENTRE) long-press -> shutdown confirm
...
unsigned long shuttingDownStart = 0;    // "Shutting Down..." splash entry time
...
// ---- BLE HID (NimBLE-based, via the installed ESP32-BLE-CompositeHID lib —
// the classic Bluedroid-based BleKeyboard/BleMouse libs don't work on C6,
// this one targets NimBLE specifically for exactly that reason) ----
BleCompositeHID bleHid("AIO-Transmitter", "AIO-TX", 100);
KeyboardDevice* bleKeyboard = nullptr;
MouseDevice* bleMouse = nullptr;
const int8_t BLE_CURSOR_STEP = 6; // pixels per loop iteration while a DPAD1 direction is held
...
bool lastDpad2States[5] = {false,false,false,false,false}; // edge detection for BLE arrow-key mode
...
void drawBleAdvertising(int yOffset);
void drawBleConnected(int yOffset);
void renderFrame();
...
void drawShuttingDown(int yOffset);
void playBootTone();
...
void setup() {
  ...
  // 800kHz caused severe lag (multi-second button delay) — both the SSD1306
  // and MCP23017 are only rated for Fast Mode (400kHz); 800kHz was pushing
  // real I2C transactions into failure/retry, and each failed transaction
  // eats up to the full 50ms Wire timeout before giving up. With 10 MCP reads
  // per readButtons() call, that compounds fast. 400kHz is still 4x the
  // default and stays within both devices' actual rated spec.
  ...
  // BLE HID devices are created once and reused for the whole runtime;
  // bleHid.begin()/.end() (called on entering/exiting BLE mode) control
  // whether it's actually advertising, not this setup.
  bleKeyboard = new KeyboardDevice();
  bleMouse = new MouseDevice();
...
// =============================================================================
void loop() {
  readMcpState(); // one I2C transaction for all 16 inputs; everything below reads the cache
  readButtons();
  ...
  bool secondaryActive = mcpBtn(SYS_LEFT) ||
                         mcpBtn(SYS_RIGHT) ||
                         (digitalRead(SYS_CENTRE) == LOW) || // native GPIO, not on the expander
                         mcpBtn(ANALOG_1_UP) ||
                         mcpBtn(ANALOG_1_DOWN) ||
                         mcpBtn(ANALOG_2_UP) ||
                         mcpBtn(ANALOG_2_DOWN);
  ...
  if (anyPress || secondaryActive) {
    ...
    if (isDisplaySleep) {
      ...
      // Screen was already forced to Home at the moment sleep was entered
      // (below), so waking just has to turn the panel back on and swallow
      // the wake press — no screen-state work needed here.
  ...
  if (!isDisplaySleep && currentScreen != PAGE_BOOT && !inhibitSleep) {
    if (millis() - lastActivityTime > 10000) {
      isDisplaySleep = true;
      // Force straight to Home the moment the screen goes dark — inhibitSleep
      // above guarantees we're always on an idle menu screen here, never mid
      // session, so this can never interrupt anything. Doing the reset here
      // (rather than on wake) means the framebuffer already holds Home by the
      // time the panel is off; nothing on the wake side can race or override it.
      // Panel is switched off BEFORE the redraw is pushed — SSD1306_DISPLAYOFF
      // only disables the panel drivers, the GDDRAM stays writable, so the
      // Home frame lands invisibly instead of flashing on screen first.
      display.ssd1306_command(SSD1306_DISPLAYOFF);
      currentScreen = PAGE_OPTIONS;
      isAnimating = false;
  ...
  bool powerBtnDown = (digitalRead(SYS_CENTRE) == LOW);
  ...
  if (!isAnimating) {
    ...
    // Update analog simulation based on MCP buttons. Stepped on a fixed
    // timer rather than once per loop iteration: loop rate varies wildly
    // (an active ESP-NOW/Smart Connect session is far slower than an idle
    // menu), so a per-iteration step made the gauges crawl on exactly the
    // screens where they matter most, which read as "analog doesn't work".
    static unsigned long lastAnalogStep = 0;
    if (millis() - lastAnalogStep >= 25) { // ~40 steps/s => full 0-100 in 2.5s
      lastAnalogStep = millis();
      if (mcpBtn(ANALOG_1_UP))   { if (simulated_analog_1 < 100) simulated_analog_1++; }
      if (mcpBtn(ANALOG_1_DOWN)) { if (simulated_analog_1 > 0)   simulated_analog_1--; }
      if (mcpBtn(ANALOG_2_UP))   { if (simulated_analog_2 < 100) simulated_analog_2++; }
      if (mcpBtn(ANALOG_2_DOWN)) { if (simulated_analog_2 > 0)   simulated_analog_2--; }
    }
    ...
    // ===== SHUTDOWN CONFIRM (power button long-press) =====
      ...
      if (buttonJustPressed[4]) {
        if (shutdownConfirmFocus == 0) {
          shuttingDownStart = millis();
          startAnimation(SCREEN_SHUTTING_DOWN, -64);
        } else {
    ...
    // ===== SHUTTING DOWN (splash before actually sleeping) =====
    else if (currentScreen == SCREEN_SHUTTING_DOWN) {
      lastActivityTime = millis();
      if (millis() - shuttingDownStart > 1400) {
    ...
    // 2x2 grid: 0=ESP_NOW 1=SMART  /  2=WiFi UDP 3=BLE   (-1=Back)
      ...
      if (buttonJustPressed[4]) {
        ...
        else if (txMenuFocus == 3) { // BLE
          // Advertised name is "{device name}-BLE" — truncate the device
          // name (not the "-BLE" suffix) so the total never exceeds 31 bytes,
          // the hard Bluetooth legacy-advertising payload limit.
          const char* SUFFIX = "-BLE";
          String base = txDeviceName;
          if (base.length() + strlen(SUFFIX) > 30) {
            base = base.substring(0, 30 - strlen(SUFFIX));
          }
          bleHid.deviceName = (base + SUFFIX).c_str();
          bleHid.begin();
          startAnimation(SCREEN_BLE_ADVERTISING, -64);
        }
      }
    }

    // ===== BLE ADVERTISING =====
    else if (currentScreen == SCREEN_BLE_ADVERTISING) {
      lastActivityTime = millis();
      bool blePowerBack = (digitalRead(SYS_CENTRE) == LOW);
      if (buttonJustPressed[2] || blePowerBack) { // Left DPAD or Power = back, cancel advertising
        bleHid.end();
        startAnimation(SCREEN_TX_MENU, 64);
      } else if (bleHid.isConnected()) {
        bleSessionActive = true;
        startAnimation(SCREEN_BLE_CONNECTED, -64);
      }
    }

    // ===== BLE CONNECTED (persistent — screen may sleep, session stays up) =====
    else if (currentScreen == SCREEN_BLE_CONNECTED) {
      lastActivityTime = millis();
      static bool lastBleCentre = false;
      static unsigned long bleCentrePressTime = 0;
      bool bleCentre = (digitalRead(SYS_CENTRE) == LOW);

      // DPAD 1 = cursor movement, continuous while held — a cursor should
      // keep moving as long as the direction is pressed, not jump once.
      int8_t mx = 0, my = 0;
      if (buttonStates[0]) my -= BLE_CURSOR_STEP; // Up
      if (buttonStates[1]) my += BLE_CURSOR_STEP; // Down
      if (buttonStates[2]) mx -= BLE_CURSOR_STEP; // Left
      if (buttonStates[3]) mx += BLE_CURSOR_STEP; // Right
      if (mx != 0 || my != 0) {
        bleMouse->mouseMove(mx, my);
        bleMouse->sendMouseReport();
      }

      // DPAD 1 Center = left click. mouseClick() in the HID library is a
      // no-op (see MouseDevice.cpp) — press/release on the actual button
      // edges is the only way to get a real press-and-hold click.
      static bool lastLeftCentre = false;
      bool leftCentre = buttonStates[4];
      if (leftCentre && !lastLeftCentre) bleMouse->mousePress();
      if (!leftCentre && lastLeftCentre) bleMouse->mouseRelease();
      lastLeftCentre = leftCentre;

      // DPAD 2 = arrow keys — discrete press/release per press+release, like
      // a real keyboard (a stuck "held" key with no release is a real bug).
      if (dpad2States[0] && !lastDpad2States[0]) { bleKeyboard->keyPress(KEY_UP); bleKeyboard->sendKeyReport(); }
      if (!dpad2States[0] && lastDpad2States[0]) { bleKeyboard->keyRelease(KEY_UP); bleKeyboard->sendKeyReport(); }
      if (dpad2States[1] && !lastDpad2States[1]) { bleKeyboard->keyPress(KEY_DOWN); bleKeyboard->sendKeyReport(); }
      if (!dpad2States[1] && lastDpad2States[1]) { bleKeyboard->keyRelease(KEY_DOWN); bleKeyboard->sendKeyReport(); }
      if (dpad2States[2] && !lastDpad2States[2]) { bleKeyboard->keyPress(KEY_LEFT); bleKeyboard->sendKeyReport(); }
      if (!dpad2States[2] && lastDpad2States[2]) { bleKeyboard->keyRelease(KEY_LEFT); bleKeyboard->sendKeyReport(); }
      if (dpad2States[3] && !lastDpad2States[3]) { bleKeyboard->keyPress(KEY_RIGHT); bleKeyboard->sendKeyReport(); }
      if (!dpad2States[3] && lastDpad2States[3]) { bleKeyboard->keyRelease(KEY_RIGHT); bleKeyboard->sendKeyReport(); }
      // DPAD 2 Center = press/select (OK/confirm)
      if (dpad2States[4] && !lastDpad2States[4]) { bleKeyboard->keyPress(KEY_ENTER); bleKeyboard->sendKeyReport(); }
      if (!dpad2States[4] && lastDpad2States[4]) { bleKeyboard->keyRelease(KEY_ENTER); bleKeyboard->sendKeyReport(); }
      for (int i = 0; i < 5; i++) lastDpad2States[i] = dpad2States[i];

      // Analog 1 = Home / Back. These use the HID Consumer (media) page, not
      // the plain keyboard page — Home/Back are navigation/media commands on
      // a remote, not text-editing keys, and consumer-page codes are what
      // TVs/media devices actually expect for these actions.
      static bool lastAnalog1Up = false, lastAnalog1Down = false;
      bool analog1Up = mcpBtn(ANALOG_1_UP);
      bool analog1Down = mcpBtn(ANALOG_1_DOWN);
      if (analog1Up && !lastAnalog1Up)     { bleKeyboard->mediaKeyPress(KEY_MEDIA_WWWHOME);   bleKeyboard->sendMediaKeyReport(); }
      if (!analog1Up && lastAnalog1Up)     { bleKeyboard->mediaKeyRelease(KEY_MEDIA_WWWHOME); bleKeyboard->sendMediaKeyReport(); }
      if (analog1Down && !lastAnalog1Down) { bleKeyboard->mediaKeyPress(KEY_MEDIA_WWWBACK);   bleKeyboard->sendMediaKeyReport(); }
      if (!analog1Down && lastAnalog1Down) { bleKeyboard->mediaKeyRelease(KEY_MEDIA_WWWBACK); bleKeyboard->sendMediaKeyReport(); }
      lastAnalog1Up = analog1Up; lastAnalog1Down = analog1Down;

      // Analog 2 = Volume Up / Down (also HID Consumer page)
      static bool lastAnalog2Up = false, lastAnalog2Down = false;
      bool analog2Up = mcpBtn(ANALOG_2_UP);
      bool analog2Down = mcpBtn(ANALOG_2_DOWN);
      if (analog2Up && !lastAnalog2Up)     { bleKeyboard->mediaKeyPress(KEY_MEDIA_VOLUMEUP);   bleKeyboard->sendMediaKeyReport(); }
      if (!analog2Up && lastAnalog2Up)     { bleKeyboard->mediaKeyRelease(KEY_MEDIA_VOLUMEUP); bleKeyboard->sendMediaKeyReport(); }
      if (analog2Down && !lastAnalog2Down) { bleKeyboard->mediaKeyPress(KEY_MEDIA_VOLUMEDOWN);   bleKeyboard->sendMediaKeyReport(); }
      if (!analog2Down && lastAnalog2Down) { bleKeyboard->mediaKeyRelease(KEY_MEDIA_VOLUMEDOWN); bleKeyboard->sendMediaKeyReport(); }
      lastAnalog2Up = analog2Up; lastAnalog2Down = analog2Down;

      // System Center/Power, held ~1s, terminates the session — same pattern
      // used by the ESP-NOW/WiFi UDP control screens.
      if (bleCentre && !lastBleCentre) bleCentrePressTime = millis();
      if (bleCentre && (millis() - bleCentrePressTime > 1000)) {
        txTerminateFocus = 1;
        startAnimation(SCREEN_TX_TERMINATE, -64);
      }
      lastBleCentre = bleCentre;
    }
    ...
    // ===== RX DISCONNECTED (telemetry lost — 5s or any press back to Page 2) =====
      ...
      bool sysLeft = mcpBtn(SYS_LEFT);
      bool sysRight = mcpBtn(SYS_RIGHT);
      bool sysCentre = (digitalRead(SYS_CENTRE) == LOW);
      ...
      if (buttonJustPressed[4]) {
        if (txTerminateFocus == 0) {
          ...
          // Resume
          if (bleSessionActive) startAnimation(SCREEN_BLE_CONNECTED, 64);
          else if (udpSessionActive) startAnimation(SCREEN_TX_UDP_CONTROL, 64);
    ...
    // The latched press edges have now had their chance to be consumed by
    // whichever screen handler ran above — clear them. Deliberately inside
    // this block, not after it: while animating, the handlers never run, so
    // the latch has to survive until they do.
    for (int i = 0; i < 5; i++) buttonJustPressed[i] = false;

  } // end !isAnimating

  // ---------- Render ----------
  // Throttled to ~30Hz. Pushing the 1KB framebuffer over I2C costs ~27ms,
  // and doing it every single iteration capped the whole loop — including
  // readButtons() — at ~30Hz. A quick tap that started and ended inside one
  // of those pushes was never sampled at all. Skipping the push on most
  // iterations lets loop() run a few hundred times a second, which is what
  // actually makes buttons (and wake-from-sleep) feel reliable.
  static unsigned long lastPush = 0;
  if (millis() - lastPush >= 33) {
    lastPush = millis();
    renderFrame(); // clears buffer + draws current/animating screen internally
    display.display();
  }
  delay(1); // yield so the background WiFi/connectivity tasks still get time
}

// Draws the current (or animating) screen into the SSD1306's RAM buffer.
// Called from loop() — steps animation progress and picks which screen(s) to
// draw, but does not itself push anything over I2C (loop() calls
// display.display() right after).
void renderFrame() {
...
void drawScreen(ScreenState screen, int yOffset, int xOffset) {
  ...
  else if (screen == SCREEN_BLE_ADVERTISING)     drawBleAdvertising(yOffset);
  else if (screen == SCREEN_BLE_CONNECTED)       drawBleConnected(yOffset);
  ...
  else if (screen == SCREEN_SHUTTING_DOWN)      drawShuttingDown(yOffset);
}
...
void drawBleAdvertising(int yOffset) {
  ...
  display.setTextColor(SSD1306_WHITE);
  int16_t x1, y1; uint16_t w, h;
  display.getTextBounds("BLE", 0, 0, &x1, &y1, &w, &h);
  display.setCursor((128 - w) / 2, 2 + yOffset);
  display.print("BLE");

  drawCenteredText(display, "ADVERTISING...", 28 + yOffset, 1);

  // Same Windows-style indeterminate ping-pong bar used by ESP-NOW/Smart
  // Connect scanning, for a consistent "something is happening" feel.
  const int barX = 18, barW = 92, barH = 8;
  const int barY = 46 + yOffset;
  display.drawRect(barX, barY, barW, barH, SSD1306_WHITE);
...
void drawBleConnected(int yOffset) {
  ...
  drawCenteredText(display, "D1: Cursor", 36 + yOffset, 1);
  drawCenteredText(display, "D2: Arrows", 48 + yOffset, 1);
  drawCenteredText(display, "Hold Center to exit", 58 + yOffset, 1);
}
...
void drawTxTelemetry(int yOffset, int xOffset) {
  ...
  for (int row = 0; row < 4 && *line; row++) {
    char segment[22];
    // Split on an embedded '\n' before the 20-char limit, if present — letting a
    // literal '\n' reach display.print() triggers Adafruit_GFX's own newline
    // handling, which resets cursor_x to 0 (ignoring our 6px/xOffset margin) and
    // slams the next word straight into the box border.
    char* nl = strchr(line, '\n');
    int len = strlen(line);
    int untilNl = nl ? (int)(nl - line) : len;
    int take = (untilNl < 20) ? untilNl : 20;
    ...
    lineY += 10;
    line += take;
    if (*line == '\n') line++; // consume the newline itself, never pass it to print()
  }
  display.setTextWrap(true);
}
...
void drawShuttingDown(int yOffset) {
  ...
  drawCenteredText(display, "Shutting Down...", 26 + yOffset, 1);

  // Same Windows-style indeterminate ping-pong bar as drawTxScanning, for a
  // consistent "something is happening" feel across the firmware.
  const int barX = 18, barW = 92, barH = 8;
  const int barY = 46 + yOffset;
  display.drawRect(barX, barY, barW, barH, SSD1306_WHITE);
...
void enterLightSleep() {
  ...
  // domain — SOC_RTCIO_PIN_COUNT). SYS_CENTRE (21) is outside that range, so a
  ...
  // ESP-IDF does not support calling esp_light_sleep_start() while the
  // Wi-Fi/ESP-NOW radio is running — it's explicitly undefined behavior, and
  // in practice it can hang the call outright instead of ever sleeping. This
  // sketch keeps the ESP-NOW radio up continuously from setup() onward, so it
  // has to be torn all the way down before sleeping and rebuilt from scratch
  // on wake (mirrors what setup() originally did) — that's what was actually
  // freezing the device right at the shutdown confirm, not on wake.
  bool wasEspNowActive = espNowInitialized;
  bool wasWifiEnabled = wifiEnabled;
  if (wasEspNowActive) {
    removeTxPeer();
    esp_now_del_peer(ESPNOW_BROADCAST);
    esp_now_unregister_recv_cb();
  ...
  gpio_wakeup_enable((gpio_num_t)SYS_CENTRE, GPIO_INTR_LOW_LEVEL);
  esp_sleep_enable_gpio_wakeup();
  // By default the C6 swaps every GPIO to its "sleep-mode" pad config on
  // sleep entry — that config has NO pullup, so SYS_CENTRE floats and a press
  ...
  gpio_sleep_sel_dis((gpio_num_t)SYS_CENTRE);
  ...
  gpio_wakeup_disable((gpio_num_t)SYS_CENTRE);
  // gpio_wakeup_enable() armed a LEVEL-triggered (not edge-triggered)
  // interrupt on this pin — gpio_wakeup_disable() alone may not fully clear
  // that pending interrupt state at the driver level. Explicitly disable the
  // GPIO interrupt too, defensively, in case leftover interrupt config here
  // is what's making later digitalRead(SYS_CENTRE) calls behave as if the
  // pin were stuck low even though the hardware measures fine.
  gpio_intr_disable((gpio_num_t)SYS_CENTRE);

  // Bring the radio back up exactly the way it was before sleep.
  if (wasEspNowActive) {
    initEspNow();
...
// =============================================================================
void readMcpState() {
  uint16_t raw = mcp.readGPIOAB();
  static uint16_t lastRaw = 0xFFFF;
  unsigned long now = millis();
  for (uint8_t i = 0; i < 16; i++) {
    uint16_t mask = 1 << i;
    if ((raw & mask) != (lastRaw & mask)) {
      mcpBitChangeTime[i] = now;                       // still bouncing, restart its timer
    } else if (now - mcpBitChangeTime[i] >= MCP_DEBOUNCE_MS) {
      if (raw & mask) mcpStable |= mask; else mcpStable &= ~mask;
    }
  }
  lastRaw = raw;
}

// True when the button on this MCP pin is pressed (active-low with pull-ups).
bool mcpBtn(uint8_t pin) { return (mcpStable & (1 << pin)) == 0; }

void readButtons() {
  bool currentUp     = mcpBtn(DPAD_1_UP);
  bool currentDown   = mcpBtn(DPAD_1_DOWN);
  bool currentLeft   = mcpBtn(DPAD_1_LEFT);
  bool currentRight  = mcpBtn(DPAD_1_RIGHT);
  bool currentCenter = mcpBtn(DPAD_1_CENTER);
  ...
  // Latched (|=), not overwritten. A press edge exists for exactly one
  // readButtons() call, but the screen handlers that consume it are skipped
  // entirely while a transition animation is running (~250ms). Overwriting
  // meant any press landing in that window was silently swallowed — a real
  // cause of "sometimes the button just doesn't work". The handlers clear
  // these once they've actually had a chance to act on them.
  ...
  dpad2States[0] = mcpBtn(DPAD_2_UP);
  dpad2States[1] = mcpBtn(DPAD_2_DOWN);
  dpad2States[2] = mcpBtn(DPAD_2_LEFT);
  dpad2States[3] = mcpBtn(DPAD_2_RIGHT);
  dpad2States[4] = mcpBtn(DPAD_2_CENTER);
}
